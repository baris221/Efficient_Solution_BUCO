...
int main(int argc, char *argv[]) {
    ...
    int *timer = malloc(nb_categories * sizeof(int));
    ...
    for (int i = 0; i < nb_items; i++) {
        ...
        if (timer[categories_by_item[i]] > nb_item_per_category[categories_by_item[i]]) {
            printf("Too many items in class %d\n", categories_by_item[i]);
            return 1;
        }

        items_per_category[categories_by_item[i]][timer[categories_by_item[i]]] = i;
        timer[categories_by_item[i]]++;
    }

    int *p=malloc((nb_categories+1)*sizeof(int));
    for(int i=0;i<nb_categories;i++){
        p[i]=0;
    }

    p[nb_categories]=sum_weight;

    //printf("ALL IZZ WELL");


    TreeNDS* res=initializationTreeNDS(nb_categories+1,nb_categories+1,4,100);
    TreeSearch(res,items_per_category,nb_item_per_category,0,nb_categories,p,weights);
